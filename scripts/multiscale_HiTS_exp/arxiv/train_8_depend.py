...
class DataSet:
    def __init__(self, train_data, val_data, test_data, dt, step_size, n_forward):
        ...
        self.train_ys = self.train_ys[:,[8,9,10,11,12,13,14,15,16]]#4, 5, 6, 7, 8, 9, 10, 11, 12]]
        ...
        self.val_ys = self.val_ys[:,[8,9,10,11,12,13,14,15,16]]
...
class ResNet(torch.nn.Module):
    ...
    def calculate_loss(self, x, ys, w=1.0):
        ...
        #8+4 = 12
        pred_mid = self.prev_models[2](pred_big)
        y_preds[:,4,:] = pred_mid
        
        #8+4+1 = 13
        y_preds[:,5,:] =  self.prev_models[0](pred_mid)
        
        #8+4+2 = 14
        fourteen = self.prev_models[1](pred_mid)
        y_preds[:,6,:] = fourteen
        
        #8+4+2+1 = 15 
        y_preds[:,7,:] =  self.prev_models[0](fourteen)
...
#make and train
model_name = 'model_D{}_noise{}_depends2.pt'.format(step_size, noise)
