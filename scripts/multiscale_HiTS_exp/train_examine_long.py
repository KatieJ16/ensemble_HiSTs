...
import Resnet_multiscale_general_examine_training_long as net
...
#     arch = [2, 128,128,128, 2]  # architecture of the neural network
    ...
    for step_size in [4]:#,8,16,32]:
        ...
        for letter in ['a']:
            #make and train
            model_name_base = 'model_long_D{}_noise{}'.format(step_size, noise)
            ...
            try:
                ...
                # create/load model object
                print('create model {} ...'.format(model_name_base))
...
#                                     model_path=os.path.join(model_dir, model_name), print_every=100, type=str(i))
            ...
            model.train_net(dataset, max_epoch=max_epoch, batch_size=batch_size, lr=lr,
                            model_dir=model_dir, model_name_base=model_name_base, print_every=10)