...
#             val_loss = self.calculate_loss(dataset.val_x, dataset.val_ys, w=w, path_list=range((epoch % num_cycles) * num_paths_per_cycle, min((epoch % num_cycles+1) * num_paths_per_cycle, len_all_combos)))
            # ================ early stopping =================
...
#                                                                               val_loss.item()))
                  print('epoch {}, training loss {}'.format(epoch, train_loss.item()))
            ...
            if epoch % 10 == 0:
                ...
                for i in range(num_cycles):
                    val_loss_sum += self.calculate_loss(dataset.val_x, dataset.val_ys, w=w, path_list=range((i % num_cycles) * num_paths_per_cycle, min((i % num_cycles+1) * num_paths_per_cycle, len_all_combos))).item()
                ...
                if val_loss_sum < self.best_loss:
                    ...
                    if model_dir is not None:
                        ...
                        print('epoch {}, training loss {}, val_loss_sum {}'.format(epoch, train_loss.item(),
                                                                                  val_loss_sum))
                        print("model_path = ", model_path_this)
                        torch.save(self, model_path_this)
                val_loss_sum = 0
#             else: 
...
#                     if model_dir is not None:
#                         model_path_this=os.path.join(model_dir, 'model_n' + str(0.1)+'_' + str(self.save_count) + '.pt')
#                         self.save_count += 1
#                         print('(--> new model saved @ epoch {})'.format(epoch))
#                         print('epoch {}, training loss {}, validation loss {}'.format(epoch, train_loss.item(),
#                                                                                   val_loss.item()))
...
#                         torch.save(self, model_path_this)
    ...
    def calculate_loss(self, x, ys, path_list=None, w=1.0):
        """
...
#         batch_size, n_dim = ys.size()
        ...
        #if no path specified, pick random ones. 
        if path_list is None:
            path_list = random.sample(range(possibilities), 25)
